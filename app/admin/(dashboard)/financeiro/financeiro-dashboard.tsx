...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  completed: { label: 'Concluída', color: 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2 bg-zinc-800/80 border border-zinc-700/50 rounded-xl px-3 py-2">
      ...
      <div className="rounded-2xl border border-zinc-700/50 bg-zinc-800/40 p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm text-zinc-400">{fmt(totalCollectedGrand)} / {fmt(totalGrand)}</span>
        </div>
        ...
        <div className="flex items-center justify-between mt-2">
          <span className="text-xs text-emerald-400 font-medium">{collectedPct.toFixed(1)}% recebido</span>
          <span className="text-xs text-yellow-400 font-medium">{fmt(totalPendingGrand)} pendente</span>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-zinc-800/60 border border-zinc-700/50 rounded-xl p-1 w-fit">
        ...
        >
          ...
          Contratos Fixos ({data.fixedPayments.length})
        </button>
      </div>
        ...
        <div className="rounded-2xl border border-zinc-700/50 bg-zinc-800/40 overflow-hidden">
          <div className="px-5 py-4 border-b border-zinc-700/50 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 text-xs text-zinc-500">
              ...
              <span>Recebido: <span className="text-emerald-400 font-semibold">{fmt(data.totalCollected)}</span></span>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-xs text-zinc-500 border-b border-zinc-700/50">
                    ...
                    <th className="px-5 py-3 text-left font-medium">Campo</th>
                    <th className="px-5 py-3 text-left font-medium">Status</th>
                    <th className="px-5 py-3 text-right font-medium">Total</th>
                    <th className="px-5 py-3 text-right font-medium">Sinal</th>
                    <th className="px-5 py-3 text-right font-medium">Restante</th>
                    <th className="px-5 py-3 text-center font-medium">Pgto. Sinal</th>
                    <th className="px-5 py-3 text-center font-medium">Pgto. Restante</th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={b.id} className={`hover:bg-zinc-700/20 transition-colors ${isCancelled ? 'opacity-50' : ''}`}>
                        ...
                        <td className="px-5 py-3.5">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-medium border ${statusInfo.color}`}>
                            {statusInfo.label}
                          </span>
                        </td>
                        <td className="px-5 py-3.5 text-right text-white font-semibold">{fmt(b.total_amount)}</td>
                        <td className="px-5 py-3.5 text-right text-zinc-300">{fmt(b.deposit_amount)}</td>
                        <td className="px-5 py-3.5 text-right text-zinc-300">{fmt(b.remaining_amount)}</td>
                        <td className="px-5 py-3.5 text-center">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-medium border ${depositInfo.color}`}>
                            {depositInfo.label}
                          </span>
                        </td>
                        <td className="px-5 py-3.5 text-center">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-medium border ${remainingInfo.color}`}>
                            {remainingInfo.label}
                          </span>
                        </td>
                      </tr>
        ...
        <div className="rounded-2xl border border-zinc-700/50 bg-zinc-800/40 overflow-hidden">
          <div className="px-5 py-4 border-b border-zinc-700/50 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 text-xs text-zinc-500">
              ...
              <span>Recebido: <span className="text-emerald-400 font-semibold">{fmt(data.fixedCollected)}</span></span>
          ...
          {data.fixedPayments.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-xs text-zinc-500 border-b border-zinc-700/50">
                    <th className="px-5 py-3 text-left font-medium">Mês Ref.</th>
                    <th className="px-5 py-3 text-left font-medium">Cliente</th>
                    <th className="px-5 py-3 text-left font-medium">Campo</th>
                    <th className="px-5 py-3 text-right font-medium">Valor</th>
                    <th className="px-5 py-3 text-center font-medium">Status</th>
                    <th className="px-5 py-3 text-left font-medium">Método</th>
                    <th className="px-5 py-3 text-left font-medium">Data Pgto.</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-700/30">
                  {data.fixedPayments.map(fp => {
                    ...
                    return (
                      <tr key={fp.id} className="hover:bg-zinc-700/20 transition-colors">
                        <td className="px-5 py-3.5 text-zinc-300">{MONTHS[parseInt(refM) - 1]}/{refY}</td>
                        ...
                        <td className="px-5 py-3.5 text-right text-white font-semibold">{fmt(fp.amount)}</td>
                        <td className="px-5 py-3.5 text-center">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-medium border ${statusInfo.color}`}>
                            {statusInfo.label}
                          </span>
