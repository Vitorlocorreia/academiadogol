    ...
    <div className="space-y-6">
        ...
        <div className="md:col-span-2 bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-6">
          <div className="flex items-start justify-between">
            ...
            {isActive ? (
              ...
              <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-zinc-800 text-zinc-500 border border-zinc-700">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-4 border-t border-zinc-800 text-sm">
            <div>
              ...
              <p className="text-white font-medium mt-1 flex items-center gap-1.5">
          ...
          {isActive && (
            <div className="pt-6 border-t border-zinc-800 flex justify-end">
              <button
                ...
                disabled={isPending}
                className="inline-flex items-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-red-500 hover:text-white border border-red-500/20 hover:border-transparent text-red-400 text-xs font-semibold rounded-xl transition-all"
              >
                {isPending ? (
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
          ...
          <div className="space-y-3 pt-2">
            ...
            <div>
              ...
              <p className="text-zinc-300 text-sm font-medium">{contract.customer?.phone}</p>
            </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h3 className="text-lg font-bold text-white">Controle de Mensalidades</h3>
          ...
          {isActive && (
            <button
              ...
              disabled={isPending}
              className="inline-flex items-center gap-1.5 px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-white text-xs font-semibold rounded-xl transition-all"
            >
              {isPending ? (
                <Loader2 className="w-3.5 h-3.5 animate-spin" />
              ) : (
                ...
                {payments.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-zinc-800/10">
                      ...
                      <td className="py-4 font-bold text-white">{amtFormatted}</td>
                      <td className="py-4">
                        {p.status === 'paid' ? (
                        ...
                        {p.status === 'pending' && (
                          <div className="inline-flex items-center gap-2">
                            <select
                              ...
                              className="px-2 py-1.5 bg-zinc-950 border border-zinc-800 focus:border-emerald-500 rounded-lg text-xs text-zinc-300 focus:outline-none"
                            >
                            ...
                            <button
                              ...
                              disabled={isPending}
                              className="px-3 py-1.5 bg-emerald-500 hover:bg-emerald-400 disabled:bg-emerald-800 text-white text-xs font-bold rounded-lg transition-all"
                            >
                              Confirmar
                            </button>
                          </div>
